%	POL5	Calcula las trayectorias articulares resultantes de emplear polinomios quínticos
%
...
%		polinomio quíntico solución para cada una de las variables articulares. La primera
...
[y,z] = size(qdd);
if (n~=m) | (n~=z) | (h~=o) | (h~=y)
   error('Las dimensiones de las matrices de especificación de posición, velocidad y aceleración deben ser idénticas');
end

if (l~=n-1)
   error('La dimensión del vector de duración de intervalos no es correcta')
end
...
for w=1:h
   ...
   qddt = [];
	for i=1:1:n-1
   	[Qcoef(i,:,w), taux, q_taux, qd_taux, qdd_taux] = pol5aux([qaux(i) qaux(i+1)],[qdaux(i) qdaux(i+1)],[qddaux(i) qddaux(i+1)],[t_ini t_ini+tij(i)],step);
