...
% loading Robot structure
try
    load('Robot_symb')
    disp('Robot structure loaded')
catch
    if ~exist('robot','var')
        robot = genebot_sym(); % The robot structure is created
        % --------------------------------------------------------
        save('Robot_symb','robot')
        disp('Robot structure computed and saved')
    end
end

% Computing CoM of each body w.r.t. frame 0
if ~isfield(robot,'CoMs_frame0')
    count = 1;
    syms d1 d2 dd dAngle
    for i=1:robot.nFrames
        if robot.PI.mass(i)~=0
            temp = robot.T(1:3,4,i)+robot.PI.CoM{i};
            CoMs_frame0{count} = subs(temp,[dd*cos(dAngle),dd*sin(dAngle)],[d1,d2]);
            count =  count+1;
        end
    end
    
    robot.CoMs_frame0 = CoMs_frame0;
    robot.Masses = transpose(nonzeros(robot.PI.mass));
    save('Robot_symb','robot')
else
    disp('CoMs_frame0 field in robot already exist')
end

% Computing CoM of the whole body w.r.t. frame 0
if ~isfield(robot,'CoM')
    disp('Computing CoMs positions...')
    n = length(robot.CoMs_frame0);
    CoM = [0;0;0];
    for i=1:n
        CoM = CoM + robot.Masses(i)*robot.CoMs_frame0{i};
    end
    CoM = CoM/robot.mass;
    simplify(CoM);
    robot.CoM = CoM;
    ...
    disp('CoM field in robot already exist')
end


% Computing the velocity of CoMs w.r.t. frame 0
if ~isfield(robot,'CoMs_velocities')
    disp('Computing CoMs velocities...')
    n = length(robot.CoMs_frame0);
    for i=1:n
        CoMVel = jacobian(robot.CoMs_frame0{i},robot.q)*robot.qD; %jacobian(pc1,q)*dq
        CoMs_velocities{i} = simplify(CoMVel);
    end
    robot.CoMs_velocities = CoMs_velocities;
    CoMVel = jacobian(robot.CoM,robot.q)*robot.qD; %jacobian(pc1,q)*dq
    robot.CoM_velocity = simplify(CoMVel);
    save('Robot_symb','robot')
else
    disp('CoMs_velocities field in robot already exist')
end


% Computing angular momentum w.r.t. CoM
if ~isfield(robot,'CoMs_velocities')
    disp('Computing CoMs velocities...')
    n = length(robot.CoMs_frame0);
    for i=1:n
...
sigma1 = m1*[pc1(1)*vc1(2)-pc1(2)*vc1(1)] + I1*va1;
sigma2 = m2*[pc2(1)*vc2(2)-pc2(2)*vc2(1)] + I2*va2;
sigma3 = m3*[pc3(1)*vc3(2)-pc3(2)*vc3(1)] + I3*va3;
sigma4 = m4*[pc4(1)*vc4(2)-pc4(2)*vc4(1)] + I4*va4;
sigma5 = m5*[pc5(1)*vc5(2)-pc5(2)*vc5(1)] + I5*va5;
sigma6 = mb1*[pcbras1(1)*vc6(2)-pcbras1(2)*vc6(1)] + Ib1*va6;
sigma7 = mb2*[pcbras2(1)*vc7(2)-pcbras2(2)*vc7(1)] + Ib2*va7;
